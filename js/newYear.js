...
function newYear() {
  ...
  // 动态设置 slider 背景图片（避免非窗口期加载图片）
  const HNY_IMGS = ['happy_new_year1.jpg', 'happy_new_year2.jpg', 'happy_new_year3.webp', 'happy_new_year4.gif'];
  const imgPrefix = location.pathname.startsWith('/en/') ? '/en/img/' : '/img/';
  document.querySelectorAll('.newYear-slider .swiper-slide').forEach((slide, i) => {
    if (HNY_IMGS[i]) slide.style.backgroundImage = `url(${imgPrefix}${HNY_IMGS[i]})`;
  });
...
function newYearSwiper() {
  var slider = document.querySelector(".newYear-slider");
  if (!slider || slider.querySelectorAll(".swiper-slide").length === 0) return;
